...
def update_story(new_story):
    global story
    story = new_story
    storyLabel["text"] = story

def get_story(choice_id):
    connection = sqlite3.connect("visual_novel_data.db")
    cursor = connection.cursor()
    cursor.execute("SELECT text, image_path FROM choices WHERE choice_id=?", (choice_id,))
    result = cursor.fetchone()
    connection.close()
    if result:
        text, image_path = result
        container["image"] = load_image(image_path)
        update_story(text)

def restart():
    container["image"] = start_image
    update_story("You meet a woman on the way to a Replit meetup IRL")
    restartButton.pack_forget()
    button.pack()
    button2.pack()

def load_image(image_path):
    return tk.PhotoImage(file=image_path).subsample(4)

start_image = load_image("visualNovel/1.png")

canvas = tk.Canvas(window, width=300, height=200)
canvas.pack()
container = canvas.create_image(150, 150, image=start_image)

storyLabel = tk.Label(text=story)
storyLabel.pack()

button = tk.Button(text="Ask her how she codes?", command=lambda: get_story(1))
button.pack()

button2 = tk.Button(text="Tell her about Replit", command=lambda: get_story(2))
button2.pack()
